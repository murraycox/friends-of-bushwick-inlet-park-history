<script lang="ts">
	...
	function onEraClick(id) {
		...
		if (navigateWithinMap) {
			...
			console.log(`NavTimeline:onEraClick(): navigating to map view ${eras[id].view}`);

			goto(`${base == '' ? '/' : `${base}/`}#/view/${encodeURIComponent(eras[id].view)}`);
		}
	}
...
<style>
	...
	/* Styles for .era-pre-colonial */
	#timeline.no-active-era circle.era-pre-1600s,
	#timeline circle.active-era.era-pre-1600s,
	.timeline-title-container.era-pre-1600s {
		fill: var(--color-era-1); /* for circle */
		background-color: var(--color-era-1); /* for div */
	}

	/* Styles for .era-early-european-settlement */
	#timeline.no-active-era circle.era-early-european-settlement,
	#timeline circle.active-era.era-early-european-settlement,
	.timeline-title-container.era-early-european-settlement {
		fill: var(--color-era-2);
		background-color: var(--color-era-2); /* for div */
	}

	/* Styles for era-urban-industrial-era */
	#timeline.no-active-era circle.era-urban-industrial,
	#timeline circle.active-era.era-urban-industrial,
	.timeline-title-container.era-urban-industrial {
		fill: var(--color-era-3);
		background-color: var(--color-era-3); /* for div */
	}

	/* Styles for .era-migration */
	#timeline.no-active-era circle.era-deindustrialization,
	#timeline circle.active-era.era-deindustrialization,
	.timeline-title-container.era-deindustrialization {
		fill: var(--color-era-4);
		background-color: var(--color-era-4); /* for div */
	}

	/* Styles for .era-activism-deindustrialization */
	#timeline.no-active-era circle.era-the-future,
	#timeline circle.active-era.era-the-future,
