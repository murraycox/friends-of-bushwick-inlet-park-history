<script lang="ts">
	...
	export function onChapterMouseOver(event) {
		...
		story.views[activeViewID].mapbox.layers.forEach((layer) => {
			if (layer.id.substring(0, 2) == 'h-') {
				...
				map.setFilter(`${activeViewID}-${layer.id}`, [
					'match',
					['get', 'chapter'],
					[event.detail.id],
					true,
					...
					if (layer.visibility != 'none') {
						if (
							...
							if (!map.getLayer(`${activeViewID}-${layer.id}`)) {
								...
								if (layer.layout) newLayer.layout = layer.layout;
	...
	let isNarrativeMinimized = $state(false);

	function toggleNarrativeSize() {
		isNarrativeMinimized = !isNarrativeMinimized;
	}
							...
							if (story.views[activeViewID].stops) {
								Object.values(story.views[activeViewID].stops).forEach((stop) => {
									...
									if (stop.heading == entry.target.innerText) {
										...
										if (stop.extent) moveMap(stop.extent);
	...
	function scrollNarrativeDown() {
		if (storyNarrative) {
			storyNarrative.lastElementChild.scrollIntoView({ behavior: 'smooth', block: 'end' });
		}
	}

	let narrativeViewport = $state();
	let isAtBottom = $state(false);

	async function waitForNarrativeImages() {
		if (!narrativeViewport) return;

		const images = Array.from(narrativeViewport.querySelectorAll('img'));
	...
	$effect(() => {
		...
		(async () => {
			...
			narrativeViewport.scrollTop = 0;
			isAtBottom = false;

			await waitForNarrativeImages();
	...
	function getNarrativeMaxScrollTop() {
		if (!narrativeViewport) return 0;

		return narrativeViewport.scrollHeight - narrativeViewport.clientHeight;
	}
	...
	function checkNarrativeScroll() {
		...
		const threshold = 50;
		const maxScrollTop = narrativeViewport.scrollHeight - narrativeViewport.clientHeight;
		const canScroll = maxScrollTop > threshold;

		isAtBottom = canScroll && narrativeViewport.scrollTop >= maxScrollTop - threshold;

		console.log({
			scrollTop: narrativeViewport.scrollTop,
			maxScrollTop,
			clientHeight: narrativeViewport.clientHeight,
			scrollHeight: narrativeViewport.scrollHeight,
			difference: maxScrollTop - narrativeViewport.scrollTop,
			isAtBottom
		});
	}
	...
	function handleDownButtonClick() {
		...
		narrativeViewport?.scrollTo({
			top: getNarrativeMaxScrollTop(),
			behavior: 'smooth'
		});
	}
</script>
...
<div
	id="map"
	class="{`view-${activeViewID}`} {`era-${activeEraID}`} {`stop-${activeStopID}`}"
...
>
	<div id="header">
		<div id="story-context-container">
			...
			{:else}
				<div id="era-long-label">The History of Bushwick Inlet Park</div>
			{/if}
		</div>
		<div id="header-logo">
	...
	{#if story.views[activeViewID].story}
		<div id="story-narrative-container" class:minimized={isNarrativeMinimized}>
			<button
				id="story-narrative-minmax-button"
				onclick={toggleNarrativeSize}
			...
			<div
				id="story-narrative-viewport"
				bind:this={narrativeViewport}
				onscroll={checkNarrativeScroll}
			>
...
<style>
	...
	#story-narrative-container.minimized {
		height: 75px;
	}

	#story-narrative-container.minimized #story-narrative-viewport {
		overflow: hidden;
	}

	#story-narrative-container.minimized #story-narrative {
		max-height: 55px;
		overflow: hidden;
	}

	#story-narrative-container.minimized #story-narrative h1 {
		white-space: nowrap;
		overflow: hidden;
	...
	#story-narrative-container.minimized #story-narrative p,
	#story-narrative-container.minimized #story-narrative h2,
	#story-narrative-container.minimized #story-narrative img {
		display: none;
	}

	#story-narrative-minmax-button {
		position: absolute;
		top: 8px;
		right: 12px;
		z-index: 1000;
	...
	.view-intro #story-narrative h1,
	:global(.view-intro #story-narrative h2),
	.view-intro #era-long-label {
		color: var(--color-intro) !important;
	}

	/* Styles for .era-pre-colonial */
	.era-pre-1600s #story-narrative h1,
	:global(.era-pre-1600s #story-narrative h2),
	.era-pre-1600s #era-long-label {
	...
	.era-early-european-settlement #story-narrative h1,
	:global(.era-early-european-settlement #story-narrative h2),
	.era-early-european-settlement #era-long-label {
	...
	.era-urban-industrial #story-narrative h1,
	:global(.era-urban-industrial #story-narrative h2),
	.era-urban-industrial #era-long-label {
	...
	.era-era-migration #story-narrative h1,
	...
	@media screen and (min-width: 768px) {
		...
		#story-context-container #era-long-label {
